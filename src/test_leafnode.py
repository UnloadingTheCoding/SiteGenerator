...
class TestLeafNode(unittest.TestCase):
    ...
    def test_no_tag(self):
        test_leaf = LeafNode("This is test text")
        self.assertEqual(test_leaf.to_html(), "This is test text")

    def test_no_value(self):
        test_leaf = LeafNode(None,"p")
        with self.assertRaises(ValueError):
            test_leaf.to_html()

    def test_no_props_format(self):
        test_leaf = LeafNode("Watching Dune part one", "p")
        self.assertEqual(test_leaf.to_html(), "<p>Watching Dune part one</p>")

    def test_with_props_format(self):
        test_leaf = LeafNode("Lets watch Dune part two", "a", {"href": "https://www.dune2movielink.org"})
